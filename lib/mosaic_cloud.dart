...
class MosaicCloud extends StatelessWidget {
  ...
  const MosaicCloud({super.key, required this.children, this.spacing = 4.0});
   ...
   Widget build(BuildContext context) {
     return CustomMultiChildLayout(
       delegate: _MosaicLayoutDelegate(childCount: children.length, spacing: spacing),
       children: [
 ...
 class _MosaicLayoutDelegate extends MultiChildLayoutDelegate {
   ...
   Rect _findNextPosition(Size childSize, List<Rect> placedRects) {
     ...
     while (true) {
       ...
       for (final rect in placedRects) {
         if (candidateRect.overlaps(rect.inflate(spacing))) { // Use the spacing parameter
           intersects = true;
           break;
         }
       }
   ...
   bool shouldRelayout(covariant _MosaicLayoutDelegate oldDelegate) {
     return oldDelegate.childCount != childCount || oldDelegate.spacing != spacing;
    }
  }